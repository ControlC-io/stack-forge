...
export function generateReadme(ctx: Ctx): string {
  return `# ${ctx.name}

${ctx.meta.description.trim() || 'TODO: one-paragraph description.'}

## Requirements

- Docker Desktop
- Node 20 (only if you run services outside Docker)
...
Open **http://127.0.0.1${ctx.meta.httpPort && ctx.meta.httpPort !== '80' ? ':' + ctx.meta.httpPort : ''}** — on Windows use \`127.0.0.1\`, not \`localhost\`.

## Deployment${ctx.hasCoolify ? `

Deployed with Coolify from \`docker-compose.coolify.yml\`.
